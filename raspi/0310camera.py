...
sensor.set_brightness(3)            # 明度の調整
sensor.set_saturation(3)            # 彩度の調整
sensor.set_contrast(3)              # コントラストの調整


#red
threshold_red = [(27, 90, 40, 127, -30, 57)] # 検出する色のしきい値を設定
#orange
threshold_orange = [(36, 67, 29, 100, -102, -33)] # 検出する色のしきい値を設定
#yellow
threshold_yellow = [(49, 90, -48, 17, 13, 127)] # 検出する色のしきい値を設定
...
while(True):
    ...
    if blobs_red :
        ...
        print("R",max_blob_red[5], max_blob_red[6],max_per_red)
    else:
        sendstr = "R0,0,0.0"
        print("R0,0,0")
    #オレンジ色を検出した場合→紫
    ...
    if blobs_orange :
        ...
        img.draw_cross(max_ xzblob_orange[5], max_blob_orange[6])
        sendstr += "," +"W"+ str(max_blob_orange[5])+","+str(max_blob_orange[6])+","+str(max_per_orange)
        print('G',max_blob_orange[5], max_blob_orange[6],max_per_orange)
    else:
        sendstr += ",W0,0,0.0"
        print("G0,0,0")
    #yellow_detect
